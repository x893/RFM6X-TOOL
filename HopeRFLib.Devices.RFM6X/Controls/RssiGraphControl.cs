      ...
      Color.Green
		...
		public void ClearAllGraphData()
		{
			try
			{
				...
				{
					if (((List<GraphPane>)this.masterPane.PaneList)[index].CurveList.Count <= 0)
						return;
					foreach (CurveItem curveItem in (List<CurveItem>)((List<GraphPane>)this.masterPane.PaneList)[index].CurveList)
					{
					...
					((List<GraphPane>)this.masterPane.PaneList)[index].AxisChange();
				}
		...
		public void UpdateLineGraph(int serie, double time, double value)
		{
			try
			{
				GraphPane graphPane = ((List<GraphPane>)this.masterPane.PaneList)[0];
				if (graphPane.CurveList.Count <= 0)
					return;
				LineItem lineItem = ((List<CurveItem>)graphPane.CurveList)[serie] as LineItem;
				if (lineItem == null)
					return;
